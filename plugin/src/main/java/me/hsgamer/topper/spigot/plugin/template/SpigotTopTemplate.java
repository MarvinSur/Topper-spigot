...
public class SpigotTopTemplate extends TopPlayerNumberTemplate implements Loadable {
    ...
    // ── Register timed queries into the existing TopQueryManager ─────────────
    //
    // Placeholder format — identical to alltime, but holder name has period suffix:
    //   %topper_money_weekly;top_name;1%
    //   %topper_money_monthly;top_value;3%
    //   %topper_money_weekly;rank%
    //   %topper_money_weekly;value%
    //
    // This works because PAPI expansion "topper" receives "money_weekly;top_name;1"
    // as params, which is then dispatched by QueryManager to our new queries below.

    private void registerTimedQueries() {
        for (TimePeriod period : TimePeriod.values()) {
            final String suffix = "_" + period.id(); // "_weekly" or "_monthly"

            // ── Timed snapshot: top_name, top_value, top_rank, top_size ──────
            getTopQueryManager().addQuery(new SnapshotQuery<UUID, Double, UUID>() {
                @Override
                protected Optional<SnapshotAgent<UUID, Double>> getAgent(@NotNull String rawName) {
                    ...
                    return getTimedAgent(holderName, period)
                            .map(a -> (SnapshotAgent<UUID, Double>) a.getSnapshotAgent());
                }
                ...
                protected Optional<SimpleQueryDisplay<UUID, Double>> getDisplay(@NotNull String rawName) {
                    if (!rawName.endsWith(suffix)) return Optional.empty();
                    String holderName = rawName.substring(0, rawName.length() - suffix.length());
                    return getTopManager().getHolder(holderName).map(NumberTopHolder::getValueDisplay);
                }
            ...
            // ── Timed personal: value, value_raw, rank, name, key ────────────
            getTopQueryManager().addQuery(new SimpleQuery<UUID, TimedCtx>() {
                {
                    registerActorAction("value", (actor, ctx) -> {
                        Double val = ctx.agent.getHolder().getEntry(actor)
                                .map(me.hsgamer.topper.data.core.DataEntry::getValue).orElse(null);
                        return ctx.display.getDisplayValue(val, ctx.sqc.args);
                    });
                    ...
                    registerActorAction("rank", (actor, ctx) -> {
                        int idx = ctx.agent.getSnapshotAgent().getSnapshotIndex(actor);
                        String fmt = ctx.sqc.args;
                        return idx < 0 ? "0" : ctx.display.getDisplayValue((double) (idx + 1), fmt);
                    });
                ...
                @Override
                protected Optional<TimedCtx> getContext(@NotNull String query) {
                    return SimpleQueryContext.fromQuery(query, false).flatMap(sqc -> {
                        ...
                        Optional<TimedAgent<UUID>> optAgent = getTimedAgent(holderName, period);
                        if (!optAgent.isPresent()) return Optional.empty();
                        return getTopManager().getHolder(holderName).map(holder ->
                                new TimedCtx(optAgent.get(), holder.getValueDisplay(), sqc));
                    });
                }
            });
        }
    ...
    private static final class TimedCtx implements SimpleQuery.Context {
        final TimedAgent<UUID> agent;
        final SimpleQueryDisplay<UUID, Double> display;
        final SimpleQueryContext sqc;

        TimedCtx(TimedAgent<UUID> agent, SimpleQueryDisplay<UUID, Double> display, SimpleQueryContext sqc) {
            this.agent = agent;
        ...
        @Override
        public @NotNull String getActionName() { return sqc.getActionName(); }
    }
    ...
    public void enable() {
        super.enable();
        // Register timed queries AFTER super.enable() so timedAgents is already populated
    ...
    public void addQueryForwardContext(Plugin plugin, String name,
            BiFunction<@Nullable UUID, @NotNull String, @NotNull QueryResult> query) {
